...
//FONT DEVICE::labelFont(std::string("/Users/i/Documents/gds-test/fonts/courier-bold.ttf"));
//FONT DEVICE::labelFont(std::string("/Users/i/Documents/gds-test/fonts/LiberationSerif-BoldItalic.ttf"));
//FONT DEVICE::labelFont(std::string("/Users/i/Documents/gds-test/fonts/fish.ttf"));
//FONT DEVICE::labelFont(std::string("/Users/i/Documents/gds-test/fonts/FabFont.ttf"));
//FONT DEVICE::labelFont(2);
...
/*
bool DEVICE::exportStructureGDS(FILE* f) {
    // BGNSTR
    putc(0x00, f);
...
//            AFFINE m = transformation;
            
//            bool isZero = m.a == 0 && m.b == 0 && m.c == 0 && m.d == 0 && m.e == 0 && m.f == 0;
//            
//            transformation.print();
...
//            printf("(f == 0) = %s\n", (m.f == 0)?("YES"):("NO"));
//            
//            printf("(a == 0) = %s\n", (m.a == (GLdouble)0.0)?("YES"):("NO"));
//            printf("(b == 0) = %s\n", (m.b == (GLdouble)0.0)?("YES"):("NO"));
//            printf("(c == 0) = %s\n", (m.c == (GLdouble)0.0)?("YES"):("NO"));
//            printf("(d == 0) = %s\n", (m.d == (GLdouble)0.0)?("YES"):("NO"));
//            printf("(e == 0) = %s\n", (m.e == (GLdouble)0.0)?("YES"):("NO"));
//            printf("(f == 0) = %s\n", (m.f == (GLdouble)0.0)?("YES"):("NO"));
//            
//            printf("(a < EPSILON) = %s\n", (m.a < EPSILON)?("YES"):("NO"));
//            printf("(b < EPSILON) = %s\n", (m.b < EPSILON)?("YES"):("NO"));
//            printf("(c < EPSILON) = %s\n", (m.c < EPSILON)?("YES"):("NO"));
...
//            printf("f = %f\n", m.f);
//            
//            printf("transformation.isZero() = %s\n", (isZero)?("YES"):("NO"));
//            printf("transformation.isZero() = %s\n", (transformation.isZero())?("YES"):("NO"));
//            printf("transformation.isIdentity() = %s\n", (transformation.isIdentity())?("YES"):("NO"));
...
/*
bool DEVICE::exportGDS(std::string fname) {
    // References:  http://www.cnf.cornell.edu/cnf_spie9.html
    //              http://www.rulabinsky.com/cavd/text/chapc.html
    //              http://boolean.klaasholwerda.nl/interface/bnf/gdsformat.html
    ...
    if (true) {
        ...
        for (int i = 0; i < polylines.size(); i++) {
            if (polylines[i].isCCW()) {
                ...
                fwrite(&(polylines[i].layer), sizeof(uint16_t), 1, f);
                ...
                std::transform(polylines[i].points.begin(), polylines[i].points.end(),
                               std::back_inserter(intPoints),
                               [dbUnitUser](VECTOR v) -> VECTORINT {
                                   return VECTORINT(v, 1/dbUnitUser);
                               });
                
                intPoints.push_back(VECTORINT(polylines[i].points[0], 1/dbUnitUser));
    ...
    printf("File %s was exported in\n\t%f seconds.\n", fname.c_str(), difftime(tend, tstart));
...
/*
bool exportGDSLibrary(std::string fname, std::string description) {
    return exportGDSLibrary(fopen(fname.c_str(), "wb"));
}
bool exportGDSLibrary(FILE* f, std::string description) {
    // References:  http://www.cnf.cornell.edu/cnf_spie9.html
    //              http://www.rulabinsky.com/cavd/text/chapc.html
    //              http://boolean.klaasholwerda.nl/interface/bnf/gdsformat.html
    ...
    if (!f) {
        printf("exportGDSLibrary(FILE*): File pointer is null. File cannot be written");
        return false;
//        throw std::runtime_error("File cannot be written");
    }
