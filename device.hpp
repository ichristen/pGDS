...
//    void unlock();
    ...
    bool exportLibraryGDS(FILE* f, FILE* txt, bool flatten=true);              // Generally for internal use only...
    
    bool exportLibraryGerber(std::string fname);                    // Export as gerber
    bool exportLibraryGerber(FILE* f);
    
private:
    bool exportStructureGDS(FILE* f, FILE* txt);                               // For internal use only...
    bool exportNoStructureGDS(FILE* f, FILE* txt, AFFINE tranformation, uint16_t level);       // For internal use only...
